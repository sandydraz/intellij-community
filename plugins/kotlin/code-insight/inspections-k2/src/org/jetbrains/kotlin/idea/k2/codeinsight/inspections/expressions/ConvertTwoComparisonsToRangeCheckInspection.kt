...
class ConvertTwoComparisonsToRangeCheckInspection : KotlinApplicableInspectionBase.Simple<KtBinaryExpression, Context>() {
    data class Context(
        ...
        fun createExpression(): KtBinaryExpression? {
            ...
            val inOperator = if (inverted) "!in" else "in"
    ...
    context(KaSession)
    private fun KaConstantValue.renderMatching(expressionType: KaType, text: String): String {
        if (this is KaConstantValue.CharValue) {
            return "'${value.renderAsEscapeSequence()}'"
        }
        ...
        return if (expressionType.isUnsignedIntegralType) {
            val uLongValue: ULong = when (this) {
                is KaConstantValue.ULongValue -> value
                is KaConstantValue.UIntValue -> value.toULong()
                is KaConstantValue.UShortValue -> value.toULong()
                is KaConstantValue.UByteValue -> value.toULong()
                else -> return render()
            }

            "${prefix}${uLongValue.toString(radix)}u"
        } else if (expressionType.isSignedIntegralType) {
            val longValue = (value as? Number)?.toLong() ?: return render()
            "${prefix}${longValue.toString(radix)}"
        } else {
            render()
        }
    }
    ...
    private fun KtExpression.adjustLowerBoundForExclusive(): String? {
        ...
        // Create the expression "$this +/- 1", then try to constant-fold it.
        val factory = KtPsiFactory(project)
        val replacementExpression = factory.createExpressionByPattern("($0 + 1)", this.copied())
        val fragment = factory.createExpressionCodeFragment(replacementExpression.text, this)

        return analyze(fragment) {
            fragment.getContentElement()?.evaluate()?.renderMatching(expressionType ?: return null, text)
                ?: fragment.text
        }
    }
    ...
    override fun createQuickFix(
        ...
        context: Context
    ): KotlinModCommandQuickFix<KtBinaryExpression> = ConvertTwoComparisonsToRangeCheckQuickFix(context)
